...
class BaseDao 
{
	...
	public function getByCondWithLimit($whereArray,$orderBy='id',$orderType='DESC',$limit,$onlyPublish = true)
	{
        $res = $this->model;
        if($onlyPublish){
        	$res = $res->where('publish',1);
        }
        return $res->where($whereArray)->orderBy($orderBy,$orderType)->limit($limit)->get();
	}
